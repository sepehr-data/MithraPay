<template>
  <div class="space-y-6" dir="rtl">
    ...
    <section class="relative overflow-hidden rounded-3xl bg-gradient-to-l from-primary/95 via-primary/85 to-secondary/80 text-primary-content p-5 sm:p-6 shadow-xl">
      <div class="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-1.5">
          ...
          <p class="mt-1 text-xs sm:text-sm text-primary-content/80">مدیریت و ویرایش مقالات آموزشی و محتوای وبلاگ سپهر‌باکس.</p>
        </div>
        <div class="flex flex-col items-end gap-2 text-[11px] sm:text-xs">
          ...
          <div class="flex flex-wrap gap-2 justify-end">
            <RouterLink to="/admin/blogs/new" class="btn btn-sm bg-primary-content text-primary rounded-2xl border-0 hover:bg-white">مطلب جدید</RouterLink>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- لیست مقالات + جستجو -->
    <section class="bg-base-100 rounded-3xl border border-base-300/80 p-4 sm:p-5 shadow-sm space-y-4 flex-grow" style="min-height: 430px;">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div class="space-y-1">
          <h3 class="text-sm sm:text-base font-semibold text-base-content">مقالات</h3>
          <p class="text-[11px] sm:text-xs text-base-content/60">لیست مقالات منتشرشده / در حال تدوین</p>
        </div>
        <div class="w-full sm:w-72 relative">
          ...
          <input v-model="q" class="input input-bordered min-h-0 h-9 w-full pr-8 rounded-2xl text-[11px] py-1" placeholder="جستجو در عنوان یا نام نویسنده..."/>
        </div>
      </div>
      ...
      <div class="space-y-3">
        <div v-if="filtered.length" class="grid gap-3 sm:gap-4 md:grid-cols-2">
          <article v-for="p in filtered" :key="p.id" class="group rounded-2xl border border-base-300 bg-base-100/90 px-3.5 py-3 sm:px-4 sm:py-3.5 flex items-center justify-between gap-3 hover:-translate-y-0.5 hover:shadow-md hover:border-primary/40 transition-all">
            <div class="flex flex-col gap-1 min-w-0">
              <h4 class="text-xs sm:text-sm font-medium text-base-content truncate max-w-[220px]">{{ p.title }}</h4>
              <div class="flex flex-wrap gap-1.5 text-[10px] sm:text-[11px]">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-base-200/80 text-base-content/70">نویسنده: <span class="ml-1">{{ p.author || 'نامشخص' }}</span></span>
                <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-base-200/60 text-base-content/70">تاریخ: <span class="ml-1">{{ p.date }}</span></span>
              </div>
            </div>
            <div class="flex flex-col items-end gap-1 text-[10px] sm:text-[11px] min-w-[120px]">
              <RouterLink :to="`/admin/blogs/${p.id}`" class="btn btn-xs btn-outline w-full justify-center">ویرایش</RouterLink>
        ...
        <div v-else class="py-10 flex flex-col items-center justify-center gap-2 text-center text-xs text-base-content/60">
          ...
          <p class="text-[11px] text-base-content/50">عبارت جستجو را تغییر بده یا یک مطلب جدید اضافه کن.</p>
        </div>
      </div>
    </section>
...
const filtered = computed(() => {
  ...
  return blogs.value.filter(b => b.title.toLowerCase().includes(t) || (b.author && b.author.toLowerCase().includes(t)))
})
...
const totalPages = computed(() => Math.ceil(filtered.value.length / blogsPerPage))
