      ...
      >
        ...
        <div class="space-y-1.5">
          ...
          <p class="mt-1 text-xs sm:text-sm text-primary-content/80">
            مدیریت بنرهای اسلایدر و بخش‌های ویژه‌ی صفحه اصلی سپهر‌باکس.
          </p>
        </div>
      ...
      <div class="space-y-3">
        ...
        >
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <!-- تصویر بنر -->
              <div
                  class="h-14 w-24 sm:h-16 sm:w-28 rounded-xl overflow-hidden bg-base-200 flex items-center justify-center"
              >
                <img
                    v-if="b.image"
                    :src="b.image"
              ...
              <div class="flex flex-col gap-1">
                <span
                    class="text-xs sm:text-sm font-medium text-base-content truncate max-w-[180px]"
                >
                  {{ b.alt || 'بدون عنوان' }}
                </span>

                <span
                    class="inline-flex items-center px-2 py-0.5 rounded-full bg-base-200/70 text-[10px] text-base-content/70"
                >
                  ID: {{ b.id }}
                </span>
              </div>
        ...
        >
          ...
          <p class="text-[11px] text-base-content/50">
            عبارت جستجو را تغییر بده یا بنر جدیدی اضافه کن.
          </p>
        </div>
      </div>
    </section>

    <!-- ادیتور بنر انتخاب‌شده -->
    <section
    ...
    >
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <label class="form-control text-right">
          <span class="label-text text-xs sm:text-sm">آدرس تصویر</span>
          <input
              v-model="banners[selectedIndex].image"
              class="input input-bordered input-sm rounded-2xl"
              placeholder="/banners/slider1.webp"
          />
        </label>

        <label class="form-control text-right">
          <span class="label-text text-xs sm:text-sm">متن جایگزین (alt)</span>
          <input
              v-model="banners[selectedIndex].alt"
              class="input input-bordered input-sm rounded-2xl"
              placeholder="عنوان کوتاه بنر"
          />
        </label>

        <label class="form-control md:col-span-2 text-right">
          <span class="label-text text-xs sm:text-sm">لینک مقصد</span>
          <input
              v-model="banners[selectedIndex].to"
              class="input input-bordered input-sm rounded-2xl"
              placeholder="/product/apple-music"
          />
        </label>
      </div>
...
// انتخاب برای ویرایش بر اساس id (نه index فیلتر شده)
const selectedId = ref<string | null>(null)

// بنرهای فیلتر شده برای نمایش
const filteredBanners = computed(() => {
  if (!q.value) return banners.value
  const t = q.value.toLowerCase()
  return banners.value.filter(
      (b) =>
          b.alt?.toLowerCase().includes(t) ||
          b.to?.toLowerCase().includes(t) ||
...
// index بنر انتخاب‌شده در آرایه اصلی
const selectedIndex = computed(() => {
  if (!selectedId.value) return -1
  return banners.value.findIndex((b) => b.id === selectedId.value)
})
