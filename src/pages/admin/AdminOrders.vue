  ...
  <div class="space-y-6" dir="rtl">
    ...
    <section class="relative overflow-hidden rounded-3xl bg-gradient-to-l from-primary/95 via-primary/85 to-secondary/80 text-primary-content p-5 sm:p-6 shadow-xl">
      <div class="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-1.5">
          ...
          <p class="mt-1 text-xs sm:text-sm text-primary-content/80">مشاهده، جستجو و مدیریت سفارشات ثبت‌شده در سپهر‌باکس.</p>
        </div>
    ...
    <section class="rounded-3xl bg-base-100 border border-base-300/80 p-4 sm:p-5 shadow-sm space-y-4">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div class="w-full sm:w-72 relative">
          ...
          <input v-model="q" class="input input-bordered min-h-0 h-9 w-full pr-8 rounded-2xl text-[11px] py-1" placeholder="جستجو بر اساس کد سفارش، شماره موبایل یا نام محصول..."/>
        </div>
      </div>
    ...
    <section class="bg-base-100 rounded-3xl border border-base-300 p-4 sm:p-5 shadow-sm space-y-3 flex-grow" style="min-height: 300px;">
      ...
      <div v-if="filtered.length" class="grid gap-3 sm:gap-4 md:grid-cols-2">
        <article v-for="o in paginatedOrders" :key="o.id" class="group rounded-2xl border border-base-300 bg-base-100/90 px-3.5 py-3 sm:px-4 sm:py-3.5 flex items-center justify-between gap-3 hover:-translate-y-0.5 hover:shadow-md hover:border-primary/40 transition-all">
          <!-- اطلاعات اصلی سفارش -->
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col gap-0.5">
              <!-- کد سفارش + محصول -->
              <div class="flex flex-col gap-0.5">
                <span class="text-[11px] sm:text-xs ltr:font-mono text-base-content/80">{{ o.id }}</span>
                <span class="text-xs sm:text-sm font-medium text-base-content truncate max-w-[200px]">{{ o.productTitle }}</span>
              </div>

              <!-- شماره موبایل کاربر -->
              <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-base-200/70 text-[11px] sm:text-xs">
                <span class="ltr font-mono text-base-content/90">{{ o.userPhone }}</span>
              </span>
            </div>
          </div>
          ...
          <div class="flex flex-col items-end gap-1 text-[10px] sm:text-[11px]">
            <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-base-200/80 text-base-content/80 whitespace-nowrap">{{ formatPrice(o.amount) }}</span>
            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-base-100 text-base-content/70 border border-dashed border-base-300 group-hover:border-primary/40">
              <span class="w-1 h-1 rounded-full" :class="statusDotClass(o.status)"></span>
      ...
      <div v-else class="py-10 flex flex-col items-center justify-center gap-2 text-center text-xs text-base-content/60">
        ...
        <p class="text-[11px] text-base-content/50">عبارت جستجو یا فیلتر بالا را تغییر بده تا سفارش‌های بیشتری نمایش داده شوند.</p>
      </div>
    </section>
...
const filtered = computed(() => {
  ...
  if (filterTab.value === 'paid') base = base.filter(o => o.status === 'paid')
  else if (filterTab.value === 'pending') base = base.filter(o => o.status !== 'paid')
  if (q.value) {
    const term = q.value.toLowerCase()
    base = base.filter(o => o.id.toLowerCase().includes(term) || o.userPhone.includes(term) || o.productTitle.toLowerCase().includes(term))
  }
  return base
})
...
const totalPages = computed(() => Math.ceil(filtered.value.length / ordersPerPage))
...
function statusLabel(status: 'paid' | 'pending' | 'canceled') {
  return status === 'paid' ? 'پرداخت شده' : status === 'pending' ? 'در انتظار' : 'لغو شده'
}

function statusDotClass(status: 'paid' | 'pending' | 'canceled') {
  return status === 'paid' ? 'bg-success/80' : status === 'pending' ? 'bg-warning/80' : 'bg-error/80'
}

function avatarInitial(o: Order) {
  return o.productTitle.trim().charAt(0) || o.id.trim().charAt(0)
}
</script>
