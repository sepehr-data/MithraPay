        ...
        <div v-else>
          ...
          <div class="p-4 sm:p-6 border-t border-base-300/60 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div class="flex gap-2">
              ...
              <button
                  v-else
                  class="btn rounded-2xl btn-ghost"
                  type="button"
                  disabled
              >
      ...
      <dialog ref="detailsModal" class="modal">
        <div class="modal-box max-w-xl rounded-3xl">
          <div class="flex items-start justify-between gap-3">
            ...
            <span v-if="selectedOrder" class="badge rounded-full mt-1" :class="statusBadgeClass(selectedOrder.status)">
...
// ✅ اضافه شد: دسترسی مستقیم به axios instance برای ست کردن هدر (workaround)
...
/** ------------------------------------
 * ✅ Workaround: ensure token + header
 * ------------------------------------ */
function readAnyToken(): string {
  // کلیدهایی که ممکنه تو پروژه‌ات ذخیره شده باشن
  const candidates = [
  ...
  // اگر به اشتباه "Bearer ..." ذخیره شده بود، حذفش کن
...
function ensureAuthForRequests(): boolean {
  ...
  if (!token) {
    // اینجا می‌تونی redirect هم بکنی
    toast.error("برای مشاهده سفارش‌ها باید وارد حساب شوید.")
    return false
  }

  // یک‌دست کردن کل پروژه: همیشه auth_token
  localStorage.setItem("auth_token", token)

  // ✅ اطمینان بیشتر: علاوه بر interceptor، مستقیم روی axios instance هم ست می‌کنیم
...
/** -----------------------------
 * ✅ API wiring: listMyOrders(limit/offset)
 * ----------------------------- */
async function fetchOrdersPage(reset: boolean) {
  if (loading.value || loadingMore.value) return

  // قبل از هر درخواست، مطمئن شو auth درست شده
  ...
  try {
    ...
    if (status === 401) {
      ...
      // پاکسازی توکن خراب برای جلوگیری از loop
      localStorage.removeItem("auth_token")
      // اگر route لاگین داری:
      // router.push("/login")
      return
    }
...
const filteredOrders = computed(() => {
  ...
  return orders.value.filter((o) => {
    ...
    const inCode = normalize((o as any).code).includes(q)
    const inItems = ((o as any).items || []).some((it: any) => normalize(it.productTitle).includes(q))
