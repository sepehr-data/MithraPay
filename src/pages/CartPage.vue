<template>
  ...
  <div v-if="cart.error" class="alert alert-error mb-4">{{ cart.error }}</div>
  <div v-if="cart.loading" class="alert">در حال دریافت سبد خرید...</div>
  <div v-else-if="cart.items.length===0" class="alert">سبد خرید شما خالی است.</div>
  <div v-else class="grid lg:grid-cols-[1fr,380px] gap-6">
    <div class="space-y-3">
      <div v-for="line in cart.detailed" :key="line.productId" class="card bg-base-100 border border-base-300">
        <div class="card-body flex-row gap-4 items-center">
          ...
          <button class="btn btn-ghost btn-sm" :disabled="cart.loading" @click="cart.remove(line.productId)">حذف</button>
        </div>
      </div>
    </div>
    <div class="card bg-base-100 border border-base-300">
      <div class="card-body">
        ...
        <RouterLink to="/checkout" class="btn btn-primary w-full" :class="cart.loading ? 'btn-disabled' : ''">ادامه خرید</RouterLink>
      </div>
    </div>
  </div>
</template>
